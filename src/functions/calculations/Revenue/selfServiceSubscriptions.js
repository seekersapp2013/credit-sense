import { selfServiceMarketingBudget } from '../SG&A/marketingAndGrowth'
import { organicTraffic } from './keyIndicators'

const { getMonthsArray, totalCols } = require('..')

const costOfAcqusitionPerAccount = ({
  model_start_date,
  self_service_business_model: { launch_date },
  self_service_paid_traffic: { expected_CAC, monthly_CAC_change, minimum_CAC_possible },
}) => {
  let data = []

  launch_date = new Date(launch_date)

  expected_CAC = parseInt(expected_CAC)
  monthly_CAC_change = parseInt(monthly_CAC_change)
  minimum_CAC_possible = parseInt(minimum_CAC_possible)

  let monthsArray = getMonthsArray(model_start_date)

  for (let i = 0; i < 84; i++) {
    let previous = i > 0 ? data[i - 1] : 0

    const val =
      monthsArray[i].getTime() === launch_date.getTime()
        ? expected_CAC
        : monthsArray[i].getTime() > launch_date.getTime()
        ? Math.max(monthly_CAC_change + previous, minimum_CAC_possible)
        : 0

    data.push(val)
  }

  return data
}

const expectedNewCustomersAdded = ({
  model_start_date,
  self_service_subscriptions,
  self_service_business_model: { launch_date },
  self_service_paid_traffic,
}) => {
  const data = []

  const selfServiceMarketingBudgetData = selfServiceMarketingBudget({
    model_start_date,
    self_service_subscriptions,
    self_service_business_model: { launch_date },
    self_service_paid_traffic,
  })

  const costOfAcqusitionPerAccountData = costOfAcqusitionPerAccount({
    model_start_date,
    self_service_business_model: { launch_date },
    self_service_paid_traffic,
  })

  for (let i = 0; i < 84; i++) {
    const val = selfServiceMarketingBudgetData[i] / costOfAcqusitionPerAccountData[i]
    data.push(val ? parseFloat(val) : 0)
  }

  return data
}

const expectedConversionsFromOrganicTraffic = ({
  model_start_date,
  website_traffic,
  self_service_subscriptions,
  self_service_business_model: { launch_date },
  self_service_conversion_to_paid,
}) => {
  const data = []

  let monthsArray = getMonthsArray(model_start_date)

  launch_date = new Date(launch_date)

  self_service_conversion_to_paid = parseInt(self_service_conversion_to_paid)

  let organicTrafficData = organicTraffic({
    model_start_date,
    website_traffic,
  })

  for (let i = 0; i < 84; i++) {
    const val =
      self_service_subscriptions && monthsArray[i].getTime() >= launch_date.getTime()
        ? organicTrafficData[i] * (self_service_conversion_to_paid / 100)
        : 0

    data.push(parseFloat(val))
  }

  return data
}

const churnRate = ({ self_service_business_model: { churn_rate } }) => {
  return Array(84).fill(parseInt(churn_rate))
}

const newCustomers = (
  {
    model_start_date,
    website_traffic,
    self_service_subscriptions,
    self_service_business_model: { billing_term, launch_date },
    self_service_paid_traffic,
    self_service_conversion_to_paid,
  },
  term,
) => {
  const data = []

  const expectedNewCustomersAddedData = expectedNewCustomersAdded({
    model_start_date,
    self_service_subscriptions,
    self_service_business_model: { launch_date },
    self_service_paid_traffic,
  })

  const expectedConversionsFromOrganicTrafficData = expectedConversionsFromOrganicTraffic({
    model_start_date,
    website_traffic,
    self_service_subscriptions,
    self_service_business_model: { launch_date },
    self_service_conversion_to_paid,
  })

  for (let i = 0; i < 84; i++) {
    const val =
      billing_term === term
        ? Math.floor(
            expectedNewCustomersAddedData[i] + expectedConversionsFromOrganicTrafficData[i],
          )
        : 0

    data.push(val)
  }

  return data
}

export const selfServiceLostAndTotalCustomers = ({
  model_start_date,
  website_traffic,
  self_service_subscriptions,
  self_service_business_model: { billing_term, launch_date, churn_rate },
  self_service_paid_traffic,
  self_service_conversion_to_paid,
}) => {
  const newCustomersData = newCustomers(
    {
      model_start_date,
      website_traffic,
      self_service_subscriptions,
      self_service_business_model: { billing_term, launch_date },
      self_service_paid_traffic,
      self_service_conversion_to_paid,
    },
    'Monthly',
  )

  const churnRateData = churnRate({ self_service_business_model: { churn_rate } })

  const lostCustomers = []

  const totalCustomers = []

  for (let i = 0; i < 84; i++) {
    const previousTotalCustomers = i > 0 ? totalCustomers[i - 1] : 0

    const lostCustomersVal = Math.round(previousTotalCustomers * (churnRateData[i] / 100))
    lostCustomers.push(lostCustomersVal ? lostCustomersVal : 0)

    const totalCustomersVal = newCustomersData[i] - lostCustomers[i] + previousTotalCustomers
    totalCustomers.push(totalCustomersVal ? totalCustomersVal : newCustomersData[i])
  }

  return [lostCustomers, totalCustomers]
}

const cohortsRenewingCustomers = ({
  model_start_date,
  website_traffic,
  self_service_subscriptions,
  self_service_business_model,
  self_service_paid_traffic,
  self_service_conversion_to_paid,
}) => {
  const data = {}

  const monthsArray = getMonthsArray(model_start_date)
  const years = []

  let i = 0

  while (i < 84) {
    years.push(monthsArray[i].getFullYear())
    i += 12
  }

  const newCustomersData = newCustomers(
    {
      model_start_date,
      website_traffic,
      self_service_subscriptions,
      self_service_business_model,
      self_service_paid_traffic,
      self_service_conversion_to_paid,
    },
    'Yearly',
  )

  const churnRateData = churnRate({ self_service_business_model })

  years.forEach((year) => {
    data[year] = []
    for (let i = 0; i < 84; i++) {
      const val =
        monthsArray[i].getFullYear() === year
          ? ''
          : monthsArray[i].getFullYear() === year + 1
          ? Math.round(i - 12 > 0 ? newCustomersData[i - 12] : 0 * (1 - churnRateData[i] / 100))
          : Math.round(i - 12 > 0 ? data[year][i - 12] : 0 * (1 - churnRateData[i] / 100))

      data[year].push(val)
    }
  })

  return data
}

const renewingCustomers = (props) => {
  const cohortsRenewingCustomersData = Object.values(cohortsRenewingCustomers(props))
  cohortsRenewingCustomersData.map((arr) => arr.map((val) => (val === '' ? 0 : val)))
  return totalCols(
    cohortsRenewingCustomersData.map((arr) => arr.map((val) => (val === '' ? 0 : val))),
  )
}

export const selfServiceAnnualTotalActiveCustomers = (props) => {
  const data = []

  const newCustomersData = newCustomers(props, 'Yearly')
  const renewingCustomersData = renewingCustomers(props)

  for (let i = 0; i < 84; i++) {
    const val =
      i + 1 <= 12
        ? newCustomersData.filter((value, j) => j === i).reduce((a, b) => a + b, 0)
        : totalCols([newCustomersData, renewingCustomersData])
            .filter((value, j) => {
              return j > i - 12 && j <= i
            })
            .reduce((a, b) => a + b, 0)
    data.push(val)
  }

  return data
}

const newMRR = (
  {
    model_start_date,
    website_traffic,
    self_service_subscriptions,
    self_service_business_model: { billing_term, launch_date, ARPU },
    self_service_paid_traffic,
    self_service_conversion_to_paid,
  },
  term,
) => {
  const data = []

  const newCustomersData = newCustomers(
    {
      model_start_date,
      website_traffic,
      self_service_subscriptions,
      self_service_business_model: { billing_term, launch_date },
      self_service_paid_traffic,
      self_service_conversion_to_paid,
    },
    term,
  )

  ARPU = parseInt(ARPU)

  for (let i = 0; i < 84; i++) {
    const val = newCustomersData[i] * ARPU
    data.push(val)
  }

  return data
}

const lostMRR = (
  {
    model_start_date,
    website_traffic,
    self_service_subscriptions,
    self_service_business_model: { billing_term, launch_date, ARPU, churn_rate },
    self_service_business_model,
    self_service_paid_traffic,
    self_service_conversion_to_paid,
  },
  term,
) => {
  const data = []

  const lostCustomersData =
    term === 'Monthly'
      ? selfServiceLostAndTotalCustomers({
          model_start_date,
          website_traffic,
          self_service_subscriptions,
          self_service_business_model: { billing_term, launch_date, churn_rate },
          self_service_paid_traffic,
          self_service_conversion_to_paid,
        })[0]
      : term === 'Yearly'
      ? renewingCustomers({
          model_start_date,
          website_traffic,
          self_service_subscriptions,
          self_service_business_model,
          self_service_paid_traffic,
          self_service_conversion_to_paid,
        })
      : Array(84).fill(0)

  ARPU = parseInt(ARPU)

  for (let i = 0; i < 84; i++) {
    const val = lostCustomersData[i] * ARPU
    data.push(val)
  }

  return data
}

const monthlyRecurringRevenue = (
  {
    model_start_date,
    website_traffic,
    self_service_subscriptions,
    self_service_business_model: { billing_term, launch_date, ARPU, churn_rate },
    self_service_business_model,
    self_service_paid_traffic,
    self_service_conversion_to_paid,
  },
  term,
) => {
  const data = []

  ARPU = parseInt(ARPU)

  const totalCustomersData =
    term === 'Monthly'
      ? selfServiceLostAndTotalCustomers({
          model_start_date,
          website_traffic,
          self_service_subscriptions,
          self_service_business_model: { billing_term, launch_date, churn_rate },
          self_service_paid_traffic,
          self_service_conversion_to_paid,
        })[1]
      : term === 'Yearly'
      ? selfServiceAnnualTotalActiveCustomers({
          model_start_date,
          website_traffic,
          self_service_subscriptions,
          self_service_paid_traffic,
          self_service_conversion_to_paid,
          self_service_business_model,
        })
      : Array(84).fill(0)

  for (let i = 0; i < 84; i++) {
    const val =
      term === 'Yearly'
        ? totalCustomersData[i] * ARPU +
          monthlyRecurringRevenue(
            {
              model_start_date,
              website_traffic,
              self_service_subscriptions,
              self_service_business_model,
              self_service_paid_traffic,
              self_service_conversion_to_paid,
            },
            'Monthly',
          )[i]
        : billing_term === 'Monthly'
        ? totalCustomersData[i] * ARPU
        : 0

    data.push(val)
  }

  return data
}

const billedCashFlow = (props) => totalCols([newMRR(props, 'Yearly'), lostMRR(props, 'Yearly')])

const LTVSubscribedThisMonth = ({
  model_start_date,
  website_traffic,
  self_service_subscriptions,
  self_service_business_model: { billing_term, ARPU, launch_date, churn_rate },
  self_service_paid_traffic,
  self_service_conversion_to_paid,
}) => {
  const data = []

  const monthlyNewCustomersData = newCustomers(
    {
      model_start_date,
      website_traffic,
      self_service_subscriptions,
      self_service_business_model: { billing_term, launch_date },
      self_service_paid_traffic,
      self_service_conversion_to_paid,
    },
    'Monthly',
  )

  const yearlyNewCustomersData = newCustomers(
    {
      model_start_date,
      website_traffic,
      self_service_subscriptions,
      self_service_business_model: { billing_term, launch_date },
      self_service_paid_traffic,
      self_service_conversion_to_paid,
    },
    'Yearly',
  )

  const churnRateData = churnRate({ self_service_business_model: { churn_rate } })

  for (let i = 0; i < 84; i++) {
    const val =
      billing_term === 'Monthly'
        ? (monthlyNewCustomersData[i] * ARPU) / (churnRateData[i] / 100)
        : billing_term === 'Yearly'
        ? (yearlyNewCustomersData[i] * ARPU) / (churnRateData[i] / 100 / 12)
        : 0

    data.push(val)
  }

  return data
}

const overallTotal = (props) =>
  totalCols([monthlyRecurringRevenue(props, 'Monthly'), billedCashFlow(props)])

export const selfServiceSubscriptionsRevenue = {
  conversions: {
    paidTraffic: {
      costOfAcqusitionPerAccount: costOfAcqusitionPerAccount,
      expectedNewCustomersAdded: expectedNewCustomersAdded,
    },
    organicTraffic: {
      expectedConversionsFromOrganicTraffic: expectedConversionsFromOrganicTraffic,
    },
  },
  monthlyBillingModel: {
    churnRate: churnRate,
    subscribers: {
      newCustomers: (props) => newCustomers(props, 'Monthly'),
      lostCustomers: (props) => selfServiceLostAndTotalCustomers(props)[0],
      totalCustomers: (props) => selfServiceLostAndTotalCustomers(props)[1],
    },
    MRR: {
      newMRR: (props) => newMRR(props, 'Monthly'),
      lostMRR: (props) => lostMRR(props, 'Monthly'),
      monthlyRecurringRevenue: (props) => monthlyRecurringRevenue(props, 'Monthly'),
    },
  },
  annualBillingModel: {
    churnRate: churnRate,
    subscribers: {
      newCustomers: (props) => newCustomers(props, 'Yearly'),
      renewingCustomers: renewingCustomers,
      totalActiveCustomers: selfServiceAnnualTotalActiveCustomers,
    },
    cohorts: {
      renewingCustomers: cohortsRenewingCustomers,
    },
    annualBillingSummary: {
      newMRR: (props) => newMRR(props, 'Yearly'),
      renewingMRR: (props) => lostMRR(props, 'Yearly'),
      billedCashFlow: billedCashFlow,
    },
  },
  totals: {
    monthlyRecurringRevenue: (props) => monthlyRecurringRevenue(props, 'Yearly'),
    LTVSubscribedThisMonth: LTVSubscribedThisMonth,
  },
  total: overallTotal,
}

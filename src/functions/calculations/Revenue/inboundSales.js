...
const costOfAcqusitionPerLead = ({
  ...
  inbound_sales_paid_traffic: { expected_CAC, monthly_CAC_change, minimum_CAC_possible },
}) => {
  let data = []
...
const expectedConversionsFromOrganicTraffic = ({
  ...
  inbound_sales,
  inbound_sales_business_model: { launch_date },
  inbound_sales_conversion_to_paid,
}) => {
  const data = []
  ...
  inbound_sales_conversion_to_paid = parseInt(inbound_sales_conversion_to_paid)
  ...
  for (let i = 0; i < 84; i++) {
    const val =
      inbound_sales && monthsArray[i].getTime() >= launch_date.getTime()
        ? organicTrafficData[i] * (inbound_sales_conversion_to_paid / 100)
        : 0
...
const agentsNeededToHandleLeads = ({
  ...
  inbound_sales_agent_lead_nurturing: { leads_per_agent },
}) => {
  const data = []
...
const churnRate = ({ inbound_sales_business_model: { churn_rate } }) => {
...
const newCustomers = (
  {
    ...
    website_traffic,
    inbound_sales,
    inbound_sales_business_model: { billing_term, launch_date },
    inbound_sales_paid_traffic,
    inbound_sales_conversion_to_paid,
    inbound_sales_lead_sale_conversion,
  },
  term,
) => {
  const data = []

  inbound_sales_lead_sale_conversion = parseInt(inbound_sales_lead_sale_conversion)
  ...
  for (let i = 0; i < 84; i++) {
    const val =
      billing_term === term
        ? Math.round(totalLeadsGeneratedData[i] + inbound_sales_lead_sale_conversion / 100)
        : 0
...
export const inboundSalesLostAndTotalCustomers = ({
  model_start_date,
  website_traffic,
  inbound_sales,
  inbound_sales_business_model: { billing_term, launch_date, churn_rate },
  inbound_sales_paid_traffic,
  inbound_sales_conversion_to_paid,
  inbound_sales_lead_sale_conversion,
}) => {
  ...
  const churnRateData = churnRate({ inbound_sales_business_model: { churn_rate } })
...
const cohortsRenewingCustomers = ({
  ...
  const churnRateData = churnRate({ inbound_sales_business_model })
...
export const inboundSalesAnnualTotalActiveCustomers = (props) => {
  const data = []
...
const newMRR = (
  {
    ...
    website_traffic,
    inbound_sales,
    inbound_sales_business_model: { billing_term, launch_date, ARPU },
    inbound_sales_paid_traffic,
    inbound_sales_conversion_to_paid,
    inbound_sales_lead_sale_conversion,
  },
...
const lostMRR = (
  {
    ...
    website_traffic,
    inbound_sales,
    inbound_sales_business_model: { billing_term, launch_date, ARPU, churn_rate },
    inbound_sales_business_model,
    inbound_sales_paid_traffic,
    inbound_sales_conversion_to_paid,
  },
...
const LTVSubscribedThisMonth = ({
  ...
  website_traffic,
  inbound_sales,
  inbound_sales_business_model: { billing_term, ARPU, launch_date, churn_rate },
  inbound_sales_paid_traffic,
  inbound_sales_conversion_to_paid,
  inbound_sales_lead_sale_conversion,
}) => {
...
export const inboundSalesRevenue = {
  conversions: {
    leadsFromPaidTraffic: {
      costOfAcqusitionPerLead: costOfAcqusitionPerLead,
      expectedLeadsAddedViaAds: expectedLeadsAddedViaAds,
    },
    leadsFromOrganicTraffic: {
      expectedConversionsFromOrganicTraffic: expectedConversionsFromOrganicTraffic,
    },
    leadHandling: {
      totalLeadsGenerated: totalLeadsGenerated,
      agentsNeededToHandleLeads: agentsNeededToHandleLeads,
    },
  },
  monthlyBillingModel: {
    churnRate: churnRate,
    subscribers: {
      ...
      lostCustomers: (props) => inboundSalesLostAndTotalCustomers(props)[0],
      totalCustomers: (props) => inboundSalesLostAndTotalCustomers(props)[1],
    },
    MRR: {
  ...
  annualBillingModel: {
    ...
    subscribers: {
      ...
      totalActiveCustomers: inboundSalesAnnualTotalActiveCustomers,
    },
    cohorts: {

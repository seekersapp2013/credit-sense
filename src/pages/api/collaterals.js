...
import { account, collateralsCollectionID, databases, profileCollectionID } from 'src/backend/appwrite'
...
const getData = async (authorization) => {
  ...
  const data = await databases.listDocuments(collateralsCollectionID, [Query.equal('user', $id)], 1)
  if (data.total > 0 && data.documents.length > 0) {
    const profile = objectifySubStrings(data.documents[0])
    // console.log(profile)
    return { profile, data: ChartsAndKPIs(profile) }
  } else {
